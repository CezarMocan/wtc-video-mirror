...
void ofxTweener::addTween(float &var, float to, float time, _cb_func_ callback){
	addTween(var,to,time, &ofxTransitions::easeOutExpo ,0,0,false, callback);
}

void ofxTweener::addTween(float &var, float to, float time, float (ofxTransitions::*ease) (float,float,float,float), _cb_func_ callback){
	addTween(var,to,time,ease,0,0,false, callback);
}
void ofxTweener::addTween(float &var, float to, float time, float (ofxTransitions::*ease) (float,float,float,float), float delay, _cb_func_ callback){
	addTween(var,to,time,ease,delay,0,false, callback);
}
void ofxTweener::addTween(float &var, float to, float time, float (ofxTransitions::*ease) (float,float,float,float), float delay, float bezierPoint, _cb_func_ callback){
	addTween(var,to,time,ease,delay, bezierPoint, true, callback);
}
	
void ofxTweener::addTween(float &var, float to, float time, float (ofxTransitions::*ease) (float,float,float,float), float delay, float bezierPoint, bool useBezier, _cb_func_ callback){
	float from = var;
	float _delay = delay;
...
void ofxTweener::update(){
	for(int i = tweens.size() -1; i >= 0; --i){
		if(float(tweens[i]._timestamp.elapsed()) >= float(tweens[i]._duration)){
			...
			float* tweenVar = tweens[i]._var;
			tweens.erase(tweens.begin() + i);

            map<float *, _cb_func_>::iterator it = callbacks.find(tweenVar);
            if(it != callbacks.end()) {
				_cb_func_ cb = it->second;
				callbacks.erase(it);
                cb(tweenVar);
            }
